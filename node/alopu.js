	...
	/** USERNAME AVAILABILITY */
		express.post('/usernamecheck', (req, res)=>{
			if(req.body.username){
				...
				.catch(err=>{
					console.error(`something went wrong looking up if an entity with username ${req.body.username} exists: `, err)
					res.json({
						success: false,
						error: err	
					})
					...
					if(args.success){
						...
						.then(args=>{
							if(args.success){
								...
								console.error('A session was not able to be created for the entity')
								res.json({
									success: false,
						...
						.catch(err=>{
							console.error('A session was not able to be created for the entity: ', err)
							res.json({
								success: false,
								error: ['A session was not able to be created for the entity', err]
							})		
						})
					} else {
						...
						entityModel.findOne({
							...
							if (err) {
								...
								let entityNew = new entityModel({
									...
									password: hash,
									email: credentials.email ? credentials.email.toLowerCase() : credentials.username + "@alopu.com"
								})
